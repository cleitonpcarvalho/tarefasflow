...
export function Topbar({ onMenuClick }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 h-14 border-b border-tf-border bg-white">
      <div className="flex h-full items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <div className="flex min-w-0 items-center gap-4">
          <div className="hidden min-w-0 text-right sm:block">
            <p className="truncate text-[13px] font-medium text-tf-text-primary">
              {user?.name ?? "Usuário"}
            </p>
          ...
          <button
            className="inline-flex h-8 items-center gap-1.5 rounded-md text-[12px] font-medium text-tf-text-muted transition hover:text-tf-text-primary"
            onClick={logout}
            type="button"
          >
